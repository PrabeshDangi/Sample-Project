...
export default function Data() {
  ...
  const handleBlock = () => {
    const cutData = document.getElementsByClassName("cut")[0];
    console.log((cutData.style.textDecoration = "line-through"));
  };
  return (
    <>
      <DataDesign>
        ...
        <div className="consumers">
          <h1>Consumer</h1>
          <button onClick={() => handleConsumer(setCondition(true))}>
            {consumer}
          </button>
          {condition ? (
            <ul>
              <li>
                <span className="cut">Andrew:</span>
                <button onClick={() => alert("This user has been suspended.")}>
                  suspend
                </button>{" "}
                <button style={{ marginLeft: "1rem" }} onClick={handleBlock}>
                  Block
                </button>
              </li>
            </ul>
